...
#define ZLET_SHELL_PARSE_BYTES(a, name)                                                            \
	{                                                                                          \
		...
		if (zlet_shell_parse_hexbytes(argv[argi], (a).name.bytes, sizeof((a).name.bytes),  \
					      &zlet_shell_len_) != 0) {                            \
			goto zlet_shell_bad;                                                       \
		}                                                                                  \
		argi++;                                                                            \
...
#define ZLET_SHELL_PARSE_FIELD(a, atype, htype, ltype, name, tag)                                  \
	ZLET_SHELL_CHECK_ATYPE_##atype(                                                            \
		ZLET_SHELL_CHECK_HTYPE_##htype(ZLET_SHELL_PARSE_##ltype(a, name)))
...
#define ZLET_SHELL_PRINT_FIELD(a, atype, htype, ltype, name, tag)                                  \
	ZLET_SHELL_CHECK_ATYPE_##atype(                                                            \
		ZLET_SHELL_CHECK_HTYPE_##htype(ZLET_SHELL_PRINT_##ltype(#name, (a).name)));
...
#define ZLET_SHELL_DEFINE_METHOD_EMPTY_RESP(_type, _instance, _name, _req_lc, _req_uc, _resp_lc,   \
	...
	{                                                                                          \
		...
		_resp_uc##_FIELDLIST(ZLET_SHELL_PRINT_FIELD, resp) return 0;                       \
	}
...
#define ZLET_SHELL_DEFINE_METHOD_REQ_EMPTY(_type, _instance, _name, _req_lc, _req_uc, _resp_lc,    \
	...
	{                                                                                          \
		...
		int rc;                                                                            \
		ARG_UNUSED(argc);                                                                  \
		_req_uc##_FIELDLIST(ZLET_SHELL_PARSE_FIELD, req) rc = _type##_##_name(             \
			&(_instance), &req, K_MSEC(CONFIG_ZEPHLETS_SHELL_CMD_TIMEOUT_MS));         \
		if (rc != 0) {                                                                     \
			shell_error(sh, "%s: %d", #_name, rc);                                     \
			return rc;                                                                 \
		}                                                                                  \
...
#define ZLET_SHELL_DEFINE_METHOD_REQ_RESP(_type, _instance, _name, _req_lc, _req_uc, _resp_lc,     \
	...
	{                                                                                          \
		...
		ARG_UNUSED(argc);                                                                  \
		_req_uc##_FIELDLIST(ZLET_SHELL_PARSE_FIELD, req) rc = _type##_##_name(             \
			&(_instance), &req, &resp, K_MSEC(CONFIG_ZEPHLETS_SHELL_CMD_TIMEOUT_MS));  \
		if (rc != 0) {                                                                     \
			shell_error(sh, "%s: %d", #_name, rc);                                     \
			return rc;                                                                 \
		}                                                                                  \
